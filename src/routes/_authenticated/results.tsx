    ...
    <div className="flex flex-col h-full">
      <div className="px-8 py-5 border-b border-border-dim">
        <p className="text-label uppercase tracking-widest text-text-muted mb-1">
          FILES BY CATEGORY /{" "}
          <span className="text-cyan-bright">{meta.breadcrumb}</span>
        </p>
...
function DuplicatesView() {
  ...
  const filteredGroups = useMemo(() => {
    let result = groups;
    if (confidenceFilter === "exact")
      result = result.filter((g) => g.confidence === "exact");
    if (confidenceFilter === "likely")
    ...
    if (search.trim()) {
      ...
      result = result.filter((g) =>
        g.files.some((f) => f.name.toLowerCase().includes(q)),
      );
    }
    ...
    return [...result].sort((a, b) => {
      ...
      if (sort === "type")
        return a.files[0].mimeType.localeCompare(b.files[0].mimeType);
      if (sort === "name")
  ...
  const allSelectedFiles: FileRecord[] = useMemo(
    () =>
      groups
        .flatMap((g) =>
          [...g.selectedForDeletion].map((id) =>
            g.files.find((f) => f.id === id),
          ),
        )
  ...
  const totalSelectedBytes = allSelectedFiles.reduce(
    (sum, f) => sum + (f.size ?? 0),
    0,
  );
  ...
  const totalRecoverableBytes = groups.reduce(
    (s, g) => s + g.totalWastedBytes,
    0,
  );
